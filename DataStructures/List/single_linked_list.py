...
def selection_sort(lista, crit):
    tam = size(lista)
    for idx in range(tam - 1):
        p_min = idx
        sig = idx + 1
        while sig < tam:
            if not crit(get_element(lista, p_min),
                        get_element(lista, sig)):
                p_min = sig
            sig += 1
        if p_min != idx:
            exchange(lista, idx, p_min)
    return lista
...
def insertion_sort(lista, crit):
    ...
    for idx in range(1, tam):
        pos = idx
        while pos > 0 and not crit(
                get_element(lista, pos - 1),
                get_element(lista, pos)):
            exchange(lista, pos, pos - 1)
            pos -= 1
    return lista


def shell_sort(lista, crit):
    tam = size(lista)
    brecha = 1
    while brecha < tam // 3:
        brecha = 3 * brecha + 1
    while brecha >= 1:
        for idx in range(brecha, tam):
            pos = idx
            while pos >= brecha and not crit(
                    get_element(lista, pos - brecha),
                    get_element(lista, pos)):
                exchange(lista, pos, pos - brecha)
                pos -= brecha
        brecha //= 3
    return lista
